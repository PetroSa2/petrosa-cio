...
try:
    ...
    logger.warning("qdrant-client not installed. QdrantVectorClient will be unavailable.")
        ...
        try:
            ...
            results = await self.client.search(
                ...
                query_filter=models.Filter(
                    must=[models.FieldCondition(key="strategy_id", match=models.MatchValue(value=strategy_id))]
                ),
                limit=limit,
                with_payload=True
            )
            if not results: return ""
            return "\n".join([f"[{res.payload.get('timestamp')}] {res.payload.get('event_type')}: {res.payload.get('summary') or res.payload.get('thought_trace')}" for res in results])
