...
from cio.core.alerting.channels import (
    AlertChannel,
    EmailChannel,
    GrafanaChannel,
    OtelChannel,
    ...
    async def dispatch(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(
                    f"Error in channel {self.channels[i].__class__.__name__}: {result}"
                )
            elif not result:
                logger.warning(
                    f"Channel {self.channels[i].__class__.__name__} failed to send alert."
                )
