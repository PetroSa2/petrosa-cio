...
async def run_simulation():
    ...
    async def mock_get(url, **kwargs):
        ...
        if "analysis/regime" in str(url):
            m.json.return_value = {
                "pair": "BTCUSDT", "metric": "regime",
                "data": {"regime": "bullish_acceleration", "volatility_level": "medium", "volume_level": "high", "trend_direction": "up", "confidence": "0.95"},
                "metadata": {"timestamp": "2026-03-08T17:00:00Z", "collection": "live"}
            }
        elif "tradeengine/state" in str(url):
            m.json.return_value = {
                "portfolio": {"net_directional_exposure": 0.1, "same_asset_pct": 0.05, "open_positions_count": 1},
                "risk_limits": {"max_drawdown_pct": 0.15, "max_orders_global": 50, "max_orders_per_symbol": 5, "max_position_size_usd": 5000.0},
                "env_stats": {"global_drawdown_pct": 0.02, "open_orders_global": 5, "open_orders_symbol": 0, "available_capital_usd": 50000.0}
            }
        elif "strategy" in str(url):
            m.json.return_value = {
                "stats": {"win_rate": 0.65, "win_rate_delta": 0.05, "consecutive_losses": 0, "recent_pnl_trend": "positive"},
                "defaults": {"stop_loss_pct": 0.02, "take_profit_pct": 0.06, "leverage": 1.0, "max_hold_hours": 12.0}
            }
        return m
    ...
    with patch("httpx.AsyncClient.get", side_effect=mock_get):
        ...
        router = OutputRouter(nats_client=mock_nc)
        listener = NATSListener(nats_client=mock_nc, orchestrator=orchestrator, context_builder=builder, router=router)
