...
from .enums import HealthStatus, RegimeFit, ActivationRecommendation, ParamChangeDirection
...
class ParamChangeSignal(BaseModel):
    ...
    reason: str = Field(..., max_length=80, description="Brief reason referencing health signals")

class AppliedParamChange(BaseModel):
    """
    Concrete parameter change after being processed by the Decision Assembler.
    Includes the old and new values.
    """
    param: str
    old_value: float
    new_value: float
...
class StrategyResult(BaseModel):
    ...
    param_change: Optional[ParamChangeSignal] = None
    thought_trace: str = Field(..., max_length=120, description="1 sentence reasoning trace")
