...
async def test_output_router_shadow_mode():
    ...
    with patch.dict(os.environ, {"DRY_RUN": "false"}):
        ...
        # Assertion: NATS publish SHOULD have been called 2 times —
        # legacy signal + decision audit copy (cio.decision.audit.execute
        # added by #610 P7.1). petrosa-cio#215 removed the dead "modern
        # trade.execute" publish (zero subscribers ecosystem-wide).
...
async def test_output_router_defaults_to_active_mode_when_dry_run_unset(
    ...
    # Legacy signal + decision audit copy (#610). petrosa-cio#215 removed the
    # dead "modern trade.execute" publish (zero subscribers ecosystem-wide).
    assert mock_nc.publish.call_count == 2
    mock_vc.upsert.assert_called_once()
