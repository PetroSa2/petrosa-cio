...
async def test_mcp_server_rate_governor_throttling():
    ...
    with patch("cio.mcp_server.discover_schema_models", return_value={}), \
         patch("cio.mcp_server.generate_tools", return_value=[{"name": "set_test", "mode": "write"}]), \
        ...
        params = {"name": "set_test", "arguments": {"payload": {}, "thought_trace": "x" * 101}}
        ...
        params = {"name": "rollback_to_version", "arguments": {"audit_id": "1", "reason": "test"}}
        result = await server._call_tool(params)
...
async def test_mcp_server_no_throttling_for_reads():
    ...
    with patch("cio.mcp_server.discover_schema_models", return_value={}), \
         patch("cio.mcp_server.generate_tools", return_value=[{"name": "get_test", "mode": "read"}]), \
         patch("cio.mcp_server.RateGovernor") as MockRateGovernor:
