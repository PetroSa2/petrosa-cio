...
from cio.core.alerting.channels import GrafanaChannel, OtelChannel, EmailChannel

@pytest.mark.asyncio
async def test_alert_manager_dispatch():
    """Test that AlertManager calls the dispatcher."""
    with patch.object(AlertManager, "_dispatcher", new_callable=AsyncMock) as mock_dispatcher:
        await AlertManager.dispatch_critical_alert("Test alert", {"key": "value"})
        mock_dispatcher.dispatch.assert_called_once_with("Test alert", {
            "alert_type": "RED",
...
async def test_grafana_channel_http_call():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        with patch.dict("os.environ", {"GRAFANA_API_URL": "http://grafana", "GRAFANA_API_KEY": "key"}):
